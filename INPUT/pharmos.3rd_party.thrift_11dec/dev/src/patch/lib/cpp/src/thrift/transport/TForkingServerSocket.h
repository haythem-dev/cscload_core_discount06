...
#ifndef _THRIFT_TRANSPORT_TFORKINGSERVERSOCKET_H_
#define _THRIFT_TRANSPORT_TFORKINGSERVERSOCKET_H_ 1
 ...
 *
 */
class TForkingServerSocket : public TServerTransport {
public:
  typedef apache::thrift::stdcxx::function<void(THRIFT_SOCKET fd)> socket_func_t;
  typedef apache::thrift::stdcxx::function<void(void)> fork_func_t;
   ...
   */
  TForkingServerSocket(int port, int sendTimeout, int recvTimeout);
   ...
   * @param port    Port number to bind to
   */
  TForkingServerSocket(const std::string& address, int port);
   ...
   */
  TForkingServerSocket(const std::string& path);

  virtual ~TForkingServerSocket();
  ...
  // forkCallback gets called from the parent after the fork happend and the currentClients counter
  // has been increased.
  void setForkCallback(const fork_func_t& forkCallback) { forkCallback_ = forkCallback; }
...
private:
  ...
  void reapChildren(int option = 0);
  ...
  int tcpRecvBuffer_;
  int maxClients_;
  volatile sig_atomic_t currentClients_;
  static const int CHILD_LIMIT_DISABLED = -1;
  bool keepAlive_;
  ...
  fork_func_t forkCallback_;
};
}
}
} // apache::thrift::transport

#endif // #ifndef _THRIFT_TRANSPORT_TForkingServerSocket_H_
