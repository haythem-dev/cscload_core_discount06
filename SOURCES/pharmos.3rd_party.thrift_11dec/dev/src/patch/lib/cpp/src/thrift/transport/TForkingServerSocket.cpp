...
#include "TForkingServerSocket.h"
...
static void destroyer_of_fine_sockets(THRIFT_SOCKET* ssock) {
...
TForkingServerSocket::TForkingServerSocket(int port)
  : port_(port),
    ...
    tcpSendBuffer_(0),
    tcpRecvBuffer_(0),
    maxClients_(TForkingServerSocket::CHILD_LIMIT_DISABLED),
    currentClients_(0),
    keepAlive_(false),
...
TForkingServerSocket::TForkingServerSocket(int port, int sendTimeout, int recvTimeout)
  : port_(port),
    serverSocket_(THRIFT_INVALID_SOCKET),
    acceptBacklog_(DEFAULT_BACKLOG),
...
TForkingServerSocket::TForkingServerSocket(const string& address, int port)
  : port_(port),
    address_(address),
    serverSocket_(THRIFT_INVALID_SOCKET),
    acceptBacklog_(DEFAULT_BACKLOG),
...
TForkingServerSocket::TForkingServerSocket(const string& path)
  : port_(0),
    path_(path),
    serverSocket_(THRIFT_INVALID_SOCKET),
    acceptBacklog_(DEFAULT_BACKLOG),
...
TForkingServerSocket::~TForkingServerSocket() {
  close();
}

void TForkingServerSocket::setSendTimeout(int sendTimeout) {
  sendTimeout_ = sendTimeout;
}

void TForkingServerSocket::setRecvTimeout(int recvTimeout) {
  recvTimeout_ = recvTimeout;
}

void TForkingServerSocket::setAcceptTimeout(int accTimeout) {
  accTimeout_ = accTimeout;
}

void TForkingServerSocket::setAcceptBacklog(int accBacklog) {
  acceptBacklog_ = accBacklog;
}

void TForkingServerSocket::setRetryLimit(int retryLimit) {
  retryLimit_ = retryLimit;
}

void TForkingServerSocket::setRetryDelay(int retryDelay) {
  retryDelay_ = retryDelay;
}

void TForkingServerSocket::setTcpSendBuffer(int tcpSendBuffer) {
  tcpSendBuffer_ = tcpSendBuffer;
}

void TForkingServerSocket::setTcpRecvBuffer(int tcpRecvBuffer) {
  tcpRecvBuffer_ = tcpRecvBuffer;
}

void TForkingServerSocket::setMaxClients(int maxClients) 
{
  maxClients_ = maxClients;
}

int TForkingServerSocket::getMaxClients() const
{
  return maxClients_;
}

int TForkingServerSocket::getCurrentClients() const
{
  return static_cast<int>(currentClients_);
}

void TForkingServerSocket::setInterruptableChildren(bool enable) {
  if (listening_) {
...
void TForkingServerSocket::listen() {
  listening_ = true;
#ifdef _WIN32
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    GlobalOutput.perror("TForkingServerSocket::listen() socketpair() interrupt", THRIFT_GET_SOCKET_ERROR);
    interruptSockWriter_ = THRIFT_INVALID_SOCKET;
    interruptSockReader_ = THRIFT_INVALID_SOCKET;
  } else {
  ...
  if (-1 == THRIFT_SOCKETPAIR(AF_LOCAL, SOCK_STREAM, 0, sv)) {
    GlobalOutput.perror("TForkingServerSocket::listen() socketpair() childInterrupt",
  ...
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
    ...
    GlobalOutput.perror("TForkingServerSocket::listen() socket() ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
...
// want to force servers to be an admin.
#ifndef _WIN32
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() THRIFT_NO_SOCKET_CACHING ",
                        errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpSendBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() SO_SNDBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (tcpRecvBuffer_ > 0) {
    if (-1 == setsockopt(serverSocket_,
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() SO_RCVBUF ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
...
// Defer accept
  ...
  if (path_.empty()) {
    if (-1 == setsockopt(serverSocket_, IPPROTO_TCP, TCP_DEFER_ACCEPT, &one, sizeof(one))) {
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() TCP_DEFER_ACCEPT ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (res->ai_family == AF_INET6 && path_.empty()) {
    ...
    if (-1 == setsockopt(serverSocket_,
                         ...
                         cast_sockopt(&zero),
                         sizeof(zero))) {
      GlobalOutput.perror("TForkingServerSocket::listen() IPV6_V6ONLY ", THRIFT_GET_SOCKET_ERROR);
    }
  }
  ...
  if (-1 == setsockopt(serverSocket_, SOL_SOCKET, SO_LINGER, cast_sockopt(&ling), sizeof(ling))) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() SO_LINGER ", errno_copy);
    close();
  ...
  if (path_.empty()) {
    ...
    if (-1
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TForkingServerSocket::listen() setsockopt() TCP_NODELAY ", errno_copy);
      close();
      throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (flags == -1) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::listen() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (-1 == THRIFT_FCNTL(serverSocket_, THRIFT_F_SETFL, flags | THRIFT_O_NONBLOCK)) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::listen() THRIFT_FCNTL() THRIFT_O_NONBLOCK ", errno_copy);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (!path_.empty()) {
    ...
    if (port_ == 0 && retries <= retryLimit_) {
      ...
      if (::getsockname(serverSocket_, &sa, &len) < 0) {
        ...
        GlobalOutput.perror("TForkingServerSocket::getPort() getsockname() ", errno_copy);
      } else {
  ...
  if (retries > retryLimit_) {
    char errbuf[1024];
    if (!path_.empty()) {
      sprintf(errbuf, "TForkingServerSocket::listen() PATH %s", path_.c_str());
    } else {
      sprintf(errbuf, "TForkingServerSocket::listen() BIND %d", port_);
    }
    GlobalOutput(errbuf);
    close();
    throw TTransportException(TTransportException::NOT_OPEN,
  ...
  if (-1 == ::listen(serverSocket_, acceptBacklog_)) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::listen() listen() ", errno_copy);
    close();
...
int TForkingServerSocket::getPort() {
  return port_;
}

shared_ptr<TTransport> TForkingServerSocket::acceptImpl() {
  if (serverSocket_ == THRIFT_INVALID_SOCKET) {
    throw TTransportException(TTransportException::NOT_OPEN, "TForkingServerSocket not listening");
  }

  struct THRIFT_POLLFD fds[2];

  const int maxEintrs = 5;
  int numEintrs = 0;

  while (true) {
    
back_to_poll_go_sue_me:
    ...
    if (CHILD_LIMIT_DISABLED != maxClients_)
    {
    ...
    if (ret < 0) {
      ...
      int errno_copy = THRIFT_GET_SOCKET_ERROR;
      GlobalOutput.perror("TForkingServerSocket::acceptImpl() THRIFT_POLL() ", errno_copy);
      ...
      if (interruptSockReader_ != THRIFT_INVALID_SOCKET && (fds[1].revents & THRIFT_POLLIN)) {
        ...
        if (-1 == recv(interruptSockReader_, cast_sockopt(&buf), sizeof(int8_t), 0)) {
          GlobalOutput.perror("TForkingServerSocket::acceptImpl() recv() interrupt ",
                              THRIFT_GET_SOCKET_ERROR);
        }
      ...
      GlobalOutput("TForkingServerSocket::acceptImpl() THRIFT_POLL 0");
  ...
  if (clientSocket == -1) {
    int errno_copy = THRIFT_GET_SOCKET_ERROR;
    GlobalOutput.perror("TForkingServerSocket::acceptImpl() ::accept() ", errno_copy);
    throw TTransportException(TTransportException::UNKNOWN, "accept()", errno_copy);
  }

  //accept succeeded, now fork
  pid_t pid = fork();
  
  if ( 0 > pid ) //fork error
  {
	  int errno_copy = THRIFT_GET_SOCKET_ERROR;
	  GlobalOutput.perror("TForkingServerSocket::acceptImpl() fork failed", errno_copy);
	  throw TTransportException(TTransportException::UNKNOWN, "fork failed", errno_copy );
  }
  ...
  else //child
  {
	  ...
	  if (flags == -1) {
		int errno_copy = THRIFT_GET_SOCKET_ERROR;
		::THRIFT_CLOSESOCKET(clientSocket);
		GlobalOutput.perror("TForkingServerSocket::acceptImpl() THRIFT_FCNTL() THRIFT_F_GETFL ", errno_copy);
		throw TTransportException(TTransportException::UNKNOWN,
	  ...
	  if (-1 == THRIFT_FCNTL(clientSocket, THRIFT_F_SETFL, flags & ~THRIFT_O_NONBLOCK)) {
		...
		::THRIFT_CLOSESOCKET(clientSocket);
		GlobalOutput
			.perror("TForkingServerSocket::acceptImpl() THRIFT_FCNTL() THRIFT_F_SETFL ~THRIFT_O_NONBLOCK ",
					errno_copy);
		throw TTransportException(TTransportException::UNKNOWN,
...
shared_ptr<TSocket> TForkingServerSocket::createSocket(THRIFT_SOCKET clientSocket) {
...
void TForkingServerSocket::notify(THRIFT_SOCKET notifySocket) {
  if (notifySocket != THRIFT_INVALID_SOCKET) {
    int8_t byte = 0;
    if (-1 == send(notifySocket, cast_sockopt(&byte), sizeof(int8_t), 0)) {
      GlobalOutput.perror("TForkingServerSocket::notify() send() ", THRIFT_GET_SOCKET_ERROR);
    }
  }
}

void TForkingServerSocket::reapChildren(int option /*= 0*/)
{
  pid_t p = 0;
  const int errno_copy = errno;
  while (true)
  {
    p = waitpid(-1, NULL, option);
  ...
  void TForkingServerSocket::interrupt()
  {
    notify(interruptSockWriter_);
  }

  void TForkingServerSocket::interruptChildren()
  {
    notify(childInterruptSockWriter_);
  }

  void TForkingServerSocket::close()
  {
    if (serverSocket_ != THRIFT_INVALID_SOCKET)
    {
